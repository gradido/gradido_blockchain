...
namespace gradido {
  ...
  public:
    ...
    CommunityContext(const data::Uuid& communityuuid, uint32_t communityIdIndex);
    ...
    inline const data::Uuid& getCommunityId() const { return mCommunityUuid; }
    inline uint32_t getCommunityIdIndex() const { return mCommunityIdIndex; }
    uint32_t getOrAddPublicKey(const data::PublicKey& publicKey);
  protected:
    data::Uuid mCommunityUuid;
