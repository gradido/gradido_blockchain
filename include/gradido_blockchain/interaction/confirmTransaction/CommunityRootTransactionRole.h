    ...
    namespace confirmTransaction {
      ...
      public:
        ...
        virtual std::vector<data::AccountBalance> calculateAccountBalances(uint64_t maxTransactionNr) const {
          ...
          return {
              data::AccountBalance(mBody->getCommunityRoot()->getAufPubkey(), GradidoUnit::zero(), communityIdIndex),
              data::AccountBalance(mBody->getCommunityRoot()->getGmwPubkey(), GradidoUnit::zero(), communityIdIndex)
          };
        }
      protected:
      };
