  ...
  if (MZ_OK == ret) {
    ...
    if (base64) {
      ...
      if (!out_fp) {
        ...
        fprintf(out_fp, "const char* const cacert_pem_base64 = \"%s\";\n", base64);
        fprintf(out_fp, "constexpr unsigned long cacert_pem_base64_len = %lu;\n", base64_len);
        fprintf(out_fp, "constexpr unsigned long cacert_pem_size = %lu;\n", size);
        fprintf(out_fp, "\n#endif // CACERT_H\n");
        fclose(out_fp);
      }
    } else {
