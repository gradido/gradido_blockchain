#include "gradido_blockchain/v3_3/InMemoryBlockchain.h"

namespace gradido {
	namespace v3_3 {
		InMemoryBlockchain::InMemoryBlockchain(const std::string& communityId)
		...
		void InMemoryBlockchain::pushGradidoTransaction(std::shared_ptr<TransactionEntry> transactionEntry)
		{
			std::lock_guard _lock(mWorkMutex);
		...
		void InMemoryBlockchain::removeGradidoTransaction(std::shared_ptr<TransactionEntry> transactionEntry)
		{
			std::lock_guard _lock(mWorkMutex);
		...
		InMemoryBlockchain::UserBalance InMemoryBlockchain::calculateUserBalanceUntil(memory::ConstBlockPtr userPubkey, Timepoint date) {
