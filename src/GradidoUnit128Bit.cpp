#include "gradido_blockchain/GradidoUnit128Bit.h"
...
GradidoUnit128Bit GradidoUnit128Bit::fromString(const std::string& stringAmount)
{
		R128 gradidoPrecise;
...
std::string GradidoUnit128Bit::toString(int precision/* = 4*/) const
{
...
double GradidoUnit128Bit::roundToPrecisionDouble(R128 gradidoUnit, uint8_t precision)
{
	if (precision > 4) {
		precision = 4;
	}
...
GradidoUnit128Bit GradidoUnit128Bit::roundToPrecision(uint8_t precision) const
{
...
GradidoUnit128Bit GradidoUnit128Bit::calculateDecay(int64_t seconds) const
{
	if (seconds == 0) return GradidoUnit128Bit(mGradidoPrecise);
...
Duration GradidoUnit128Bit::calculateDecayDurationSeconds(Timepoint startTime, Timepoint endTime)
{
