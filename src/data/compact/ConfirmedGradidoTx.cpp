...
namespace gradido::data::compact {
  ...
  ConfirmedGradidoTx ConfirmedGradidoTx::fromGrdw(
    ...
    uint32_t senderCommunityIdIndex = blockchainCommunityIdIndex;
    uint32_t recipientCommunityIdIndex = blockchainCommunityIdIndex;
    if (confirmedTx.crossGroupType != CrossGroupType::LOCAL) {
      switch (confirmedTx.crossGroupType) {
      ...
      default: 
        throw GradidoUnhandledEnum(
          ...
          enum_type_name<decltype(confirmedTx.crossGroupType)>().data(), 
          enum_name(confirmedTx.crossGroupType).data()
        );
      }
    }
    switch (confirmedTx.transactionType) {
    case TransactionType::TRANSFER:
      confirmedTx.specific.transfer = TransferTx::fromGrdw(body->data.transfer, senderCommunityIdIndex, recipientCommunityIdIndex);
      break;
    case TransactionType::CREATION:
      confirmedTx.specific.creation = {
    ...
    case TransactionType::REGISTER_ADDRESS:
      ...
      confirmedTx.specific.registerAddress = {
    ...
    case TransactionType::DEFERRED_TRANSFER:
      ...
      confirmedTx.specific.deferredTransfer = {
        .amountGddCent = body->data.deferred_transfer->transfer.sender.amount,
        .senderPublicKeyIndex = g_appContext->getOrAddPublicKeyIndex(blockchainCommunityIdIndex, body->data.deferred_transfer->transfer.sender.pubkey),
        .recipientPublicKeyIndex = g_appContext->getOrAddPublicKeyIndex(blockchainCommunityIdIndex, body->data.deferred_transfer->transfer.recipient),
        .timeoutDurationSeconds = body->data.deferred_transfer->timeout_duration
      };
      break;
    case TransactionType::REDEEM_DEFERRED_TRANSFER:
      confirmedTx.specific.redeemDeferredTransfer = 
        RedeemDeferredTransferTx::fromGrdw(body->data.redeem_deferred_transfer, senderCommunityIdIndex, recipientCommunityIdIndex)
        ;
      break;
    case TransactionType::TIMEOUT_DEFERRED_TRANSFER: 
      confirmedTx.specific.timeoutDeferredTransfer = {
      ...
      break;
    case TransactionType::COMMUNITY_ROOT: 
      confirmedTx.specific.communityRoot = {
      ...
      break;
    default: 
      throw GradidoUnhandledEnum("on ConfirmedGradidoTx::fromGrdw", "TransactionType", enum_name(confirmedTx.transactionType).data());
    }
    if (loadColdData) {
      confirmedTx.coldData = ConfirmedGradidoTxCold::fromGrdw(tx, body, blockchainCommunityIdIndex);
    }
    return confirmedTx;
  }
