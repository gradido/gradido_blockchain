...
namespace model {
	namespace gradido {
		...
		TransactionBodyPtr TransactionBody::load(const std::string& protoMessageBin, std::shared_ptr<ProtobufArenaMemory> arenaMemory)
		{
			auto obj = std::make_shared<TransactionBody>(arenaMemory);
		...
		TransactionBodyPtr TransactionBody::createGroupFriendsUpdate(bool colorFusion)
		{
			auto obj = std::make_shared<TransactionBody>();
		...
		TransactionBodyPtr TransactionBody::createRegisterAddress(
		...
		TransactionBodyPtr TransactionBody::createTransactionCreation(std::unique_ptr<proto::gradido::TransferAmount> transferAmount, Timepoint targetDate)
		{
			auto obj = std::make_shared<TransactionBody>();
		...
		TransactionBodyPtr TransactionBody::createTransactionTransfer(std::unique_ptr<proto::gradido::TransferAmount> transferAmount, const MemoryBin* recipientPubkey)
		{
			auto obj = std::make_shared<TransactionBody>();
		...
		TransactionBodyPtr TransactionBody::createCommunityRoot(const MemoryBin* pubkey, const MemoryBin* gmwPubkey, const MemoryBin* aufPubkey)
		{
			auto obj = std::make_shared<TransactionBody>();
