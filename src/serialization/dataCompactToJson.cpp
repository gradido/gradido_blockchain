...
namespace serialization { 
	DEFINE_TO_JSON(CommunityRootTx, {
		...
		if (publicKey) {
			obj.AddMember("pubkey", toJson(publicKey->convertToHex(), alloc), alloc);
		}

		auto gmwPubkkey = g_appContext->getPublicKey(value.gmwPublicKeyIndex);
		if (gmwPubkkey) {
			obj.AddMember("gmwPubkey", toJson(gmwPubkkey->convertToHex(), alloc), alloc);
		}

		auto aufPubkey = g_appContext->getPublicKey(value.aufPublicKeyIndex);
		if (aufPubkey) {
			obj.AddMember("aufPubkey", toJson(aufPubkey->convertToHex(), alloc), alloc);
		}
	})
	...
	DEFINE_TO_JSON(RegisterAddressTx, {
		...
		if (accountPublicKey) {
			obj.AddMember("accountPubkey", toJson(accountPublicKey->convertToHex(), alloc), alloc);
		}
