...
std::shared_ptr<KeyPairEd25519> KeyPairEd25519::create(const memory::Block &seed)
{
	...
	auto chainCode = make_shared<memory::Block>(ED25519_CHAIN_CODE_SIZE, &I[32]);
	return std::make_shared<KeyPairEd25519>(publicKey, privateKey, chainCode);
}
/*
// ed25519-BIP0032
...
memory::Block KeyPairEd25519::calculatePublicKey(const memory::Block &privateKey)
{
	...
	memory::Block pubkey(ED25519_PUBLIC_KEY_SIZE);
	uint8_t privateKeyBuffer[64];
	crypto_sign_seed_keypair(pubkey.data(), privateKeyBuffer, privateKey.data());
	return pubkey;
}
...
memory::Block KeyPairEd25519::sign(const unsigned char *message, size_t messageSize) const
{
	if (!mExtendedSecret || mExtendedSecret->size() != ED25519_PRIVATE_KEY_SIZE){
		throw Ed25519MissingKeyException("missing or invalid secret key for sign");
	}

	if (!message || messageSize == 0) {
		throw Ed25519SignException("invalid message for sign", mSodiumPublic, "empty/missing message");
	}

	memory::Block signature(ED25519_SIGNATURE_SIZE);
	if (crypto_sign_detached(signature.data(), nullptr, message, messageSize, mExtendedSecret->data()) != 0) {
		throw Ed25519SignException("signing failed", mSodiumPublic, "libsodium signing function returned non-zero");
	}
	return signature;
}
...
{
	assert(key.size() >= 32);
	return (key[0] & 0b11111000) == key[0] && ((key[31] & 0b00011111) | 0b01000000) == key[31];
}
...
/*
// ed25519-BIP0032
// https://docs.rs/ed25519-bip32/latest/src/ed25519_bip32/derivation/mod.rs.html
...
std::shared_ptr<KeyPairEd25519Ex> KeyPairEd25519::derivePrivateKey(uint32_t index) const
{
    ...
    // key
    std::memcpy(data + 1, mExtendedSecret->data(), 32);
    ...
    crypto_auth_hmacsha512_state state;
    crypto_auth_hmacsha512_init(&state, mChainCode->data(), 32);
...
std::shared_ptr<KeyPairEd25519Ex> KeyPairEd25519::derivePublicKey(uint32_t index) const
{
	// we don't support public key derivation because it is not possible to implement it with ed25519 and the security implications of the only possible implementation are not acceptable
	throw Ed25519DeriveException("public key derivation is not supported for ed25519", mSodiumPublic);
}
